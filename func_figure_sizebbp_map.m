function func_figure_sizebbp_map(bbp,lat,lon)
...
st=0;
for imon=[1, 7]
    bbpart_2d=bbp.plk(:,:,imon);%-(bb_phyto(:,:,imon)+bb_bact(:,:,imon)+bb_zoo(:,:,imon));
    for j=1:3
        if j==1
            bbphyto_2d=bbp.pico(:,:,imon);
        elseif j==2
            bbphyto_2d=bbp.nano(:,:,imon);
        elseif j==3
            bbphyto_2d=bbp.micro(:,:,imon);
        elseif j==4
...
%         caxis([0 0.4])
        ...
        if j==1
            if imon==1
            ttl =title({strcat([lettervec{st},'. ','Pico-'])});
            elseif imon>1
...
%                 h.Ticks=0:10:50;
        ...
        elseif j==2
            if imon==1
            ttl =title({strcat([lettervec{st},'. ','Nano-'])});
            elseif imon>1
                ttl =title({strcat([lettervec{st}])});
%                 h=colorbar('horizontal');
...
%                 h.Ticks=0:10:50;
        ...
        elseif j==3
            if imon==1
            ttl =title({strcat([lettervec{st},'. ','Micro'])});
            elseif imon>1
                ttl =title({strcat([lettervec{st}])});
%                 h=colorbar('horizontal');
%                 h.Ticks=0:25:100;
