function func_figure_percentdiff_maps(biom_phyto,bb_tot,idx_out,md,bathy,lat,lon)
...
st=1;
figure
tiledlayout(4,2,'Padding','Compact');%,'TileSpacing','Compact','Padding','Compact'
...
%     chl_2d=chl_tot_t(:,:,imon);
%     biom_2d(chl_2d<minchl)=NaN;    
%     bbpart_2d(chl_2d<minchl)=NaN;
    
%     mod_darwin=md_bbp.Coefficients.Estimate(1)+bbpart_2d.*md_bbp.Coefficients.Estimate(2);
    mod_darwin=md.int+bbpart_2d.*md.slope;
    mod_darwin(bathy<500)=NaN;
    mod_darwin(mod_darwin<0)=NaN;    
%     [~, ~,logbs, logMAE]=func_calc_bias(mod_darwin(lat_2d>minlat2 & lat_2d<maxlat2),biom_2d(lat_2d>minlat2 & lat_2d<maxlat2));
    [~, ~,~, logMAE2]=func_calc_bias(mod_darwin(:),biom_2d(:));
%     Rsq=func_Rsq(log10(biom_2d(lat_2d>minlat2 & lat_2d<maxlat2)),...
%         log10(mod_darwin(lat_2d>minlat2 & lat_2d<maxlat2)));    
...
ttl =title(strcat([...
        strvec_letters{st},...
        ' R^2=',num2str(Rsq2,'%.2f'),...
        ', MAE{log}=',num2str(logMAE2,'%.2f')]),'fontweight','normal');    
